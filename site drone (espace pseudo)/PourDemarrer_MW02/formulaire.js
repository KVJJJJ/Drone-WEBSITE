...
document.getElementById("pseudo").addEventListener("input", VerifierPseudo);
function VerifierPseudo(event)
{       var Valpseudo = document.getElementById("pseudo").value;
        var nospace = 0;

        for( var i=0; i<Valpseudo.length; i++ )
        {
        if( Valpseudo.charAt(i)=='32')
        {
            nospace++;
        }
        }
        if ( nospace>=1 )
        {
            alert("le pseudo ne doit pas contenir d'espace");
            event.preventDefault();
        }
